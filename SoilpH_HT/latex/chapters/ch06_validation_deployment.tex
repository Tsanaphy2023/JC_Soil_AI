...
\section{ระบบตรวจสอบความใช้ได้ภาคสนามและสร้างชุดข้อมูลการเรียนรู้ (Field Validation \& AI Dataset Module)}

เพื่อตอบสนองต่อ\textbf{วัตถุประสงค์การวิจัยข้อที่ 1 และ 3} แอปพลิเคชัน \textbf{SoilpH-HT} ได้รับการติดตั้งหน้าต่างการทำงานเฉพาะทาง \texttt{FieldValidationScreen} ซึ่งเข้าถึงได้โดยตรงจากแดชบอร์ดหลักและเมนูการตั้งค่า โดยแบ่งออกเป็น 2 ระบบย่อยที่ทำงานประสานกัน:

\begin{enumerate}
    \item \textbf{แท็บที่ 1: การตรวจสอบเปรียบเทียบในแปลงจริง (Field Validation \& Benchmarking)}
    บันทึกผลการวัดเปรียบเทียบระหว่างชุดทดสอบพกพาภาคสนาม (Portable Kit) กับเครื่องมือวัดมาตรฐานห้องปฏิบัติการ (Lab Benchtop Meter) พร้อมระบบคำนวณสถิติสดบนหน้าจอ ได้แก่ RMSE, MAE, \%Bias, สัมประสิทธิ์การตัดสินใจ $R^2$ และระบบตรวจสอบการผ่านเกณฑ์สากล ISO/IEC 17025 \& EURACHEM (\%Bias $< 1.0\%$) พร้อมปุ่มส่งออกข้อมูลเป็นไฟล์ตารางคำนวณ CSV
    \item \textbf{แท็บที่ 2: ระบบสร้างชุดข้อมูลการเรียนรู้ปัญญาประดิษฐ์ (AI Dataset Builder)}
    สำหรับบันทึกและสร้างชุดข้อมูลตามวัตถุประสงค์ข้อที่ 1 ประกอบด้วยทูเพิล $(E_{\text{sensor}}\text{ [mV]}, T_{\text{soil}}, \text{pH}_{\text{standard}}, \text{pH}_{\text{raw}}, M_{\text{soil}}, EC)$ โดยมีปุ่มลัดบันทึกสารมาตรฐาน NIST Buffer อัตโนมัติ (pH 4.01 พาทาเลต, pH 7.00 ฟอสเฟต, pH 10.01 คาร์บอเนต) พร้อมฟังก์ชันคำนวณค่าศักย์ไฟฟ้าทฤษฎี $E_{\text{Nernst}}$ และความชัน $S_N(T)$ แบบเรียลไทม์
\end{enumerate}
...
\section{การทดสอบเปรียบเทียบในแปลงปลูกทุเรียนจริง (Objective 3)}

เพื่อตอบสนองต่อ\textbf{วัตถุประสงค์การวิจัยข้อที่ 3} ได้ทำการทดสอบเปรียบเทียบสมรรถนะของชุดทดสอบค่าความเป็นกรด-ด่างของดินแบบพกพาภาคสนามที่มีการชดเชยความคลาดเคลื่อนโดยปัญญาประดิษฐ์ โดยเปรียบเทียบความแม่นยำกับเครื่องมือวัดมาตรฐานห้องปฏิบัติการ (Lab Benchtop pH Meter รุ่น Metrohm 913 ร่วมกับโพรบแก้ว Unitrode พร้อมเซนเซอร์ Pt1000) ในแปลงปลูกทุเรียนหมอนทองและชะนีในพื้นที่ตัวแทนจังหวัดจันทบุรีและจังหวัดตราด จำนวน 8 แหล่งปลูกสำคัญ รวม 30 ตัวอย่าง ผลการทดสอบเปรียบเทียบ 8 จุดตัวแทนแสดงในตารางที่ \ref{tab:durian_field_validation_samples}

\begin{table}[H]
\centering
\caption{ผลการตรวจวัดเปรียบเทียบในแปลงปลูกทุเรียนจังหวัดจันทบุรีและตราด ระหว่างแล็บมาตรฐานกับชุดทดสอบพกพาใน 3 โหมด}
\label{tab:durian_field_validation_samples}
\resizebox{\textwidth}{!}{%
\begin{tabularx}{1.18\textwidth}{p{2.6cm}p{2.8cm}cccYYc}
    \toprule
    \textbf{รหัสแปลง} & \textbf{แหล่งปลูก / พิกัด} & \textbf{อุณหภูมิ ($^\circ\text{C}$)} & \textbf{ความชื้น (\%)} & \textbf{แล็บมาตรฐาน} & \textbf{Raw เซนเซอร์} & \textbf{Conventional ATC} & \textbf{SoilpH-HT AI} \\
    \midrule
    CHAN-TM-01 & ท่าใหม่ จันทบุรี & 29.4 & 34.2 & 5.62 & 5.38 (-0.24) & 5.51 (-0.11) & \textbf{5.64 (+0.02)} \\
    CHAN-KK-02 & เขาคิชฌกูฏ จันทบุรี & 27.8 & 42.1 & 5.45 & 5.15 (-0.30) & 5.32 (-0.13) & \textbf{5.47 (+0.02)} \\
    CHAN-KL-03 & ขลุง จันทบุรี & 31.2 & 28.5 & 6.18 & 5.80 (-0.38) & 6.02 (-0.16) & \textbf{6.19 (+0.01)} \\
    CHAN-MK-04 & มะขาม จันทบุรี & 30.1 & 31.0 & 5.88 & 5.52 (-0.36) & 5.73 (-0.15) & \textbf{5.89 (+0.01)} \\
    TRAT-KS-01 & เขาสมิง ตราด & 32.5 & 26.4 & 4.92 & 4.45 (-0.47) & 4.71 (-0.21) & \textbf{4.95 (+0.03)} \\
    TRAT-BR-02 & บ่อไร่ ตราด & 28.2 & 38.6 & 5.30 & 5.01 (-0.29) & 5.18 (-0.12) & \textbf{5.31 (+0.01)} \\
    TRAT-LO-03 & แหลมงอบ ตราด & 33.1 & 24.8 & 6.42 & 5.92 (-0.50) & 6.20 (-0.22) & \textbf{6.44 (+0.02)} \\
    TRAT-KS-04 & เขาสมิง (แปลง 2) & 29.8 & 33.5 & 5.15 & 4.82 (-0.33) & 5.01 (-0.14) & \textbf{5.17 (+0.02)} \\
    \midrule
    \multicolumn{4}{l}{\textbf{สถิติรวมประเมินสมรรถนะ (30 แปลงทดสอบ)}} & \textbf{เกณฑ์มาตรฐาน} & \textbf{Raw} & \textbf{Conv. ATC} & \textbf{Full AI PINN} \\
    \multicolumn{4}{l}{ค่ารากที่สองความคลาดเคลื่อนกำลังสองเฉลี่ย (RMSE)} & $< 0.10\text{ pH}$ & 0.384 & 0.218 & \textbf{0.038} \\
    \multicolumn{4}{l}{ค่าเฉลี่ยความคลาดเคลื่อนสัมบูรณ์ (MAE)} & $< 0.10\text{ pH}$ & 0.312 & 0.175 & \textbf{0.029} \\
    \multicolumn{4}{l}{ร้อยละความเอนเอียงเฉลี่ย (\%Bias)} & $< 1.00\%$ & 6.42\% & 3.85\% & \textbf{0.52\%} \\
    \multicolumn{4}{l}{สัมประสิทธิ์การตัดสินใจ ($R^2$)} & $> 0.980$ & 0.842 & 0.926 & \textbf{0.9985} \\
    \multicolumn{4}{l}{การยอมรับตามเกณฑ์สากล ISO/IEC 17025 \& EURACHEM} & Pass / Fail & ไม่ผ่าน & ไม่ผ่าน & \textbf{ผ่านสมบูรณ์} \\
    \bottomrule
\end{tabularx}%
}
\end{table}
