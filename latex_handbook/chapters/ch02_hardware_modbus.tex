...
\begin{enumerate}
    ...
    \item สถาปัตยกรรมหัววัดดิน 8-in-1 อุตสาหกรรม และการจ่ายพลังงานผ่าน USB OTG
    ...
    \item การเข้าสายสัญญาณและผังรีจิสเตอร์ข้อมูลมาตรฐาน
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \item ต่อวงจรสายสัญญาณระหว่างหัววัดดิน 8-in-1 กับสายแปลง USB-RS485 Type-C ได้อย่างแม่นยำ
...
\begin{enumerate}
    \item การบรรยายเชิงทฤษฎีควบคู่กับการใช้เครื่องออสซิลโลสโคปแสดงรูปคลื่นสัญญาณเชิงผลต่าง
...
\begin{enumerate}
    ...
    \item ชุดหัววัดดิน 8-in-1 RS485 และสายแปลง USB Type-C OTG
...
\begin{tikzpicture}[
    ...
    \node[box, fill=green!10, minimum width=3.2cm, right=1.0cm of bus] (probe) {
        \textbf{หัววัดดินจริง 8-in-1}\\[2pt]
        \footnotesize
        $\bullet$ Modbus RTU Slave\\
        $\bullet$ ปลายเข็มสแตนเลส 316\\
        $\bullet$ บอร์ด MCU ภายใน
    };
    ...
    \node[rectangle, rounded corners=2mm, draw=rbruGold!80, fill=rbruGold!8, line width=0.8pt, minimum width=14.5cm, minimum height=0.85cm, below=0.7cm of adapter, xshift=1.5cm, align=center] (proto) {
        \small\textbf{โปรโตคอล Modbus RTU:} 4,800 bps, 8 Data Bits, No Parity, 1 Stop Bit, CRC-16 Modbus Verification
    };
    \draw[arrow, dashed] (adapter.south) -- ++(0,-0.4) -| (proto.north);
\end{tikzpicture}
\caption{ผังการเชื่อมโยงทางกายภาพและการไหลของสัญญาณดิจิทัลผ่านพอร์ต USB Type-C OTG สู่หัววัดดินจริง}
...
\section{สถาปัตยกรรมหัววัด 8-in-1 และการจ่ายพลังงานผ่าน USB OTG}

หัววัดดิน 8-in-1 ผลิตจากสแตนเลสสตีลเกรด 316L ทนทานต่อการกัดกร่อนจากกรดและเกลือในดิน การเชื่อมต่อผ่านสายแปลง USB-RS485 Type-C ช่วยให้สมาร์ตโฟนสามารถจ่ายไฟเลี้ยง +5V DC จากพอร์ต USB ไปเลี้ยงหัววัดและวงจรประมวลผลได้โดยตรง

\begin{table}[H]
\centering
\caption{การกำหนดรหัสสีสายสัญญาณและหน้าที่ของหัววัดดิน 8-in-1}
...
\begin{tabularx}{\textwidth}{p{2.5cm}p{3.5cm}Y}
    ...
    \midrule
    สีน้ำตาล & VCC (+5V ถึง +12V DC) & ต่อเข้ากับขั้วบวกแหล่งจ่ายไฟจากสายแปลง USB OTG \\
    สีดำ & GND (0V) & ต่อเข้ากับขั้วลบหรือกราวด์ร่วมของระบบ \\
    สีเหลือง & RS485 Data A+ & ขั้วสัญญาณข้อมูลแบบไม่กลับเฟส (Non-inverting) \\
    สีน้ำเงิน & RS485 Data B- & ขั้วสัญญาณข้อมูลแบบกลับเฟส (Inverting) \\
    \bottomrule
\end{tabularx}
\end{table}
...
1. กำหนดค่าเริ่มต้นของรีจิสเตอร์ CRC ขนาด 16 บิต เท่ากับ $\mathtt{0xFFFF}$
2. นำไบต์ข้อมูลไบต์แรกมาทำการ Exclusive-OR (XOR) กับไบต์ต่ำของรีจิสเตอร์ CRC
3. เลื่อนบิตรีจิสเตอร์ CRC ไปทางขวา 1 บิต (Shift Right)
4. ตรวจสอบบิตที่หลุดออกมา (Least Significant Bit หรือ LSB)
   - หากบิตที่หลุดออกมาเป็น 1 ให้ทำการ XOR รีจิสเตอร์ CRC กับพหุนาม $\mathtt{0xA001}$
   - หากบิตที่หลุดออกมาเป็น 0 ไม่ต้องทำการ XOR
5. ทำซ้ำขั้นตอนที่ 3 และ 4 จนครบ 8 ครั้ง (ครบ 1 ไบต์ข้อมูล)
6. ทำซ้ำขั้นตอนที่ 2 ถึง 5 สำหรับไบต์ข้อมูลถัดไปจนครบทุกไบต์ในเฟรม
7. ผลลัพธ์สุดท้ายคือค่า CRC-16 โดยต้องส่งไบต์ต่ำออกไปก่อนไบต์สูงตามมาตรฐาน Little-Endian
